...
class MassWatchScraper:
    ...
    def get_pending_targets(self, all_targets: list, progress: dict) -> list:
        ...
        for target in all_targets:
            if (
                target.watch_id not in completed_ids
                and target.watch_id not in failed_ids
            ):
    ...
    def scrape_with_progress(self, targets: list) -> dict:
        ...
        for brand_name, brand_targets in brand_groups.items():
            ...
            for i, target in enumerate(brand_targets, 1):
                logger.info(
                    f"[{len(progress['completed']) + len(progress['failed']) + 1}/{len(targets)}] "
                ...
                try:
                    ...
                    if success:
                        ...
                        logger.info(
                            f"Successfully scraped {target.brand} {target.model_name}"
                        )
                    else:
                        progress["failed"].append(target.watch_id)
                        logger.warning(
                            f"Failed to scrape {target.brand} {target.model_name}"
                        )

                except Exception as e:
                    progress["failed"].append(target.watch_id)
                    logger.error(
                        f"Error scraping {target.brand} {target.model_name}: {e}"
                    )
    ...
    def print_final_report(self, progress: dict, targets: list):
        ...
        for brand, stats in brand_stats.items():
            ...
            print(
                f"  {brand}: {stats['completed']}/{stats['total']} "
    ...
    def run(self):
        ...
        try:
            ...
            if failed_count > 0:
                ...
                retry = (
                    input("Would you like to retry failed watches? (y/n): ")
                    .lower()
                    .strip()
                )
