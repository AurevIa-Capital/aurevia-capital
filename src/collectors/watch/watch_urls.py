...
class WatchDiscovery(BaseScraper):
    ...
    def process_target(self, target: Dict, **kwargs) -> bool:
        ...
        return (
            len(self.discover_watches_from_brand_page(brand, brand_url, target_count))
            > 0
        )

    def discover_watches_from_brand_page(
        self, brand: str, brand_url: str, target_count: int = 10
    ) -> List[Dict]:
    ...
    def save_watch_targets(
        self, watches: List[Dict], filename: str = "watch_targets_100.json"
    ):
...
def main():
    ...
    if watches:
        ...
        print("Target: 100 watches (10 per brand)")
